...
public abstract class QueueSearch {
	...
	protected HeuristicFunction h = null; // Para monitorizar los valores de f de los nodos expandidos
	...
	public List<Action> search(Problem problem, Queue<Node> frontier) {
		...
		// Monitorizes f-values for expanded nodes
		AStarEvaluationFunction f = null;
		if (h != null) f = new AStarEvaluationFunction(h);
		...
		while (!isFrontierEmpty() && !CancelableThread.currIsCanceled()) {
			...
			// Check for f-value monotorization
			if (f != null)
				System.out.println(f.f(nodeToExpand));
